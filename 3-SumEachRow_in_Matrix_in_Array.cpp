...
void PrintEachRowSum(stMatrix3X3 Matrix3X3, short Rows, short Cols)
{
    cout << "\nThe following are the sum of each row in th matrix:\n";
        for (short i = 0; i < Rows; i++)
        {
            cout << " Row " << i + 1 << " Sum = " << Matrix3X3.arrSum[i] << endl;
        }
}

void SumMatixRowsInArry(stMatrix3X3& Matrix3X3, short Rows, short Cols)
{
    for (short i = 0; i < Rows; i++)
    {
        Matrix3X3.arrSum[i] = RowSum(Matrix3X3, i, Cols);
    }
}
...
int main()
{
    ...
    SumMatixRowsInArry(Matrix3X3, 3, 3);
